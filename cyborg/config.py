...
# Температура слияния fuse_ideas (ультра-режим): строгое следование JSON-схеме каркаса
# важнее творческой раскованности — на 0.9 модель выдумывала свои названия ролей и
# бросала хвостовые поля (живые прогоны 2026-08-15). Тот же приём, что у score_llm.
FUSE_TEMPERATURE = 0.3
...
# Потолок на ОДИН пакет (сек). Зависший тест (сети/замки/тайминги) иначе вешает весь
# прогон навсегда без диагностики — с потолком раннер убивает пакет и репортит таймаут
# (council 2026-08-17). Живой пакет идёт ~90с; 600с = запас x6 на медленной машине.
TEST_RUNNER_TIMEOUT_SEC = 600
...
REDDIT_RSS_TOP_URL = (
    "https://www.reddit.com/r/SideProject/top/.rss?t=day"  # Atom-fallback: .rss анти-бот reddit пропускает
)
