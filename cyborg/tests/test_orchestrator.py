...
def base_organs():
    return [
        Organ(
            "collect_source",
            "собрать свежие идеи источник",
            src,
            role="source",
            produces=["items"],
            tags=["идеи", "собрать", "свежие"],
        ),
        Organ(
            "ideate",
            "придумать идеи предложить",
            xf,
            role="transform",
            produces=["ideas"],
            consumes=["items"],
...
class TestOrchestrator(unittest.TestCase):
    ...
    def test_router_selects_relevant_subset(self):
        many = base_organs() + [
            Organ("noise%d" % i, "нерелевантный шум", lambda i, e: {}, tags=["zzz"]) for i in range(12)
        ]
    ...
    def test_failing_organ_no_crash_not_retried(self):
        orgs = [Organ("bad", "собрать идеи источник", boom, role="source", produces=["items"], tags=["идеи"])]
    ...
    def test_safe_mode_gates_prod_organ(self):
        ...
        orgs = [Organ("proddy", "идеи", prod, role="source", produces=["ideas"], tags=["идеи"], needs={"prod": True})]
    ...
    def test_empty_transform_finishes(self):
        ...
        orgs = [
            Organ("collect_source", "собрать идеи источник", src, role="source", produces=["items"], tags=["идеи"]),
            Organ(
                "ideate",
                "придумать идеи",
                empty_xf,
    ...
    def test_no_progress_organ_blocked(self):
        ...
        orgs = [
            Organ("collect_source", "собрать идеи источник", wrong, role="source", produces=["items"], tags=["идеи"])
        ]
        out = Cyborg(orgs, max_steps=8).run("приноси идеи", env={})
    ...
    def test_on_step_callback_streams_live_progress(self):
        ...
        out = Cyborg(base_organs(), max_steps=6).run(
            "приноси свежие идеи", env={}, on_step=lambda step, phase, name, why: events.append((phase, name))
        )
    ...
    def test_degraded_source_not_blocked(self):
        ...
        orgs = [
            ...
            Organ(
                "ideate", "придумать идеи", xf, role="transform", produces=["ideas"], consumes=["items"], tags=["идеи"]
            ),
        ]
