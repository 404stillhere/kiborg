...
class TestReadabilityGate(unittest.TestCase):
    ...
    def test_rewrite_reverted_when_not_better(self):
        ...
        out = readability_gate.run(
            {"ideas_best": IDEAS}, {"llm": _llm('{"scores":[3,9]}', rescore='{"scores":[3,2]}'), "min_score": 7}
        )
        ideas = out["ideas_polished"]
    ...
    def test_no_fix_when_rewrite_returns_identical(self):
        ...
        out = readability_gate.run(
            {"ideas_best": IDEAS}, {"llm": _llm('{"scores":[3,9]}', why_new=same), "min_score": 7}
        )
        ideas = out["ideas_polished"]
    ...
    def test_keeps_old_why_when_rewrite_fails(self):
        ...
        out = readability_gate.run(
            {"ideas_best": IDEAS}, {"llm": _llm('{"scores":[2,8]}', why_new="извините не могу"), "min_score": 7}
        )
        ideas = out["ideas_polished"]
    ...
    def test_on_progress_emits_live_substeps(self):
        ...
        msgs = []
        out = readability_gate.run(
            {"ideas_best": IDEAS}, {"llm": _llm('{"scores":[3,9]}'), "min_score": 7, "on_progress": msgs.append}
        )
