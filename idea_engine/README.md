...
> сырьё, `env["files_paths"]`; передаёт путь, первую смысловую строку и короткий фрагмент содержимого;
> секреты/мусор отсеивает, объём ограничивает). Этот демо-срез намеренно пинит один
...
- Органы пакета покрыты своими модулями: `test_collect_source` (сетевые ленты + локальный `files`:
  честный degrade без ложного `error`, per_n-split, telegram payload, ограниченный контекст файлов,
  скрытие секретов), `test_ideate`, `test_rank_ideas`,
