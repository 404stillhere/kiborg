...
_ROLE_ALIASES = (
    (
        "механизм",
        ("механизм", "алгоритм", "приём", "прием", "хранител", "туннел", "обертк", "обёртк", "способ", "как работает"),
    ),
...
def run(inputs, env):
    ...
    for attempt in range(1 + max(0, int(inputs.get("repair_retries", 1)))):
        attempts += 1
        raw = ask(
            prompt if attempt == 0 else REPAIR_TMPL.format(violations="; ".join(violations), previous=last_raw[:1500])
        )
        last_raw = raw or ""
        violations = []
