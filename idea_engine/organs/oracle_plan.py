...
4. files — только файлы из списка "Файлы". Если шаг касается нескольких — перечисли все.
5. effort — S (<2ч), M (полдня), L (день+).
6. depends_on — id предыдущих шагов, без циклов.
7. verification — конкретная команда или наблюдение (не "убедиться", а "pytest X" или "открыть /api/state").
8. risks — реальные риски, связанные с проектом (не общие).
9. warnings — если цель требует файла, которого нет в карте.

Ответь ОДНОЙ строкой JSON compact. Пример:
{{"title":"Добавить авторизацию","summary":"Встроить JWT-аутентификацию в API и пульт.","steps":[{{"id":"S1","title":"Изучить точки входа","description":"Прочитать panel/serve.py и cyborg/run.py, найти где создаётся HTTP-сервер.","files":["panel/serve.py","cyborg/run.py"],"effort":"S","depends_on":[],"verification":"cat panel/serve.py | grep 'def run'"}}],"risks":["Секреты могут попасть в логи"],"warnings":[]}}
