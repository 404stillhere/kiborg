...
Контекст kiborg:
- Пульт (panel) — локальный веб-UI на http://127.0.0.1:8737, эндпоинт статуса /api/state.
...
4. files — только файлы из списка "Файлы". НЕ придумывай пути. НЕ указывай README.md как рабочий файл, если задача не в редактировании документации. НЕ указывай файлы данных (*.json в data/) как цели правок.
...
7. verification — конкретная команда или наблюдение. Для API пульта используй порт 8737: curl -s http://127.0.0.1:8737/api/state | grep ... . Для тестов: pytest <file>::<test>. Для UI: открыть http://127.0.0.1:8737 и увидеть изменение.
