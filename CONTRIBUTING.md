...
## Стиль user-facing сообщений

Правила для сообщений, которые видит пользователь (CLI, пульт, логи):

- Эмодзи:
  - `⚠️` — предупреждение, пользователь может продолжить;
  - `ℹ️` — информация о процессе;
  - `🛑` — критическая ошибка, дальше нельзя;
  - `⏳` — ожидание / прогресс.
- Язык: русский (основной), английский в скобках для технических терминов.
- Формат: `{эмодзи} {сообщение}` без точки в конце.
- Примеры:
  - `⚠️ Порт 8737 уже занят`
  - `ℹ️ Сканирование... 45 сек, найдено 500 файлов`
  - `🛑 Критическая ошибка: state.json повреждён`
