...
Telegram (через личный аккаунт) · локальные папки (`files` — путь, первая смысловая строка и
ограниченный безопасный фрагмент файла как сырьё; папки задаются в пульте, секреты отсеиваются).
Какие ленты ВКЛЮЧЕНЫ — юзер решает тумблерами в пульте (`cyborg/feeds.py`,
