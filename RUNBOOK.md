...
**АВТО-ВОССТАНОВЛЕНИЕ (срабатывает автоматически в начале каждого прогона):**

`harvest_runner.main()` перед органами зовёт `recover_state.auto_recover_state_if_needed()`:
если state.json повреждён/отсутствует и есть валидный бэкап — он восстанавливается автоматически,
а в Telegram (если настроен `KIBORG_ALERT_TOKEN`/`KIBORG_ALERT_CHAT_ID`) уходит CRITICAL-алерт:

```
[kiborg][CRITICAL] state.json был повреждён, автоматически восстановлен из бэкапа
2026-07-21_100000. Повреждённый файл сохранён как state.json.corrupted-<TS> для разбора.
```

Прогон при этом продолжается как ни в чём не бывало (с откатнувшимся состоянием).

**Что делать (если пришёл алерт):**
1. Убедиться, что авто-восстановление прошло успешно — проверить `/api/health`:
   ```bash
   curl http://127.0.0.1:8737/api/health
   ```
   `state_json.ok: true` и `state_json.error: null` — восстановление сработало.
2. Изучить повреждённый файл (для разбора причины — диск полный / крах записи / ручная правка):
   ```bash
   ls -la idea_engine/data/state.json.corrupted-*  # самый свежий дамп
   cat idea_engine/data/state.json.corrupted-<последний-TS>
   ```
3. Если повреждение повторяется — проверить место на диске и права на `idea_engine/data/`.
4. После разбора .corrupted-<TS> файлы можно удалить вручную (kiborg их не чистит — это
   диагностические дампы, оставляет решение об удалении за админом).

**Ручное восстановление (если авто не сработало — нет бэкапа / все тоже битые):**
1. Посмотреть список доступных бэкапов:
   ```bash
   python cyborg/restore_backup.py --list
   ```
2. Неинтерактивное авто-восстановление (та же логика, что в начале прогона — только если
   state.json повреждён; удобно для cron/скриптов):
   ```bash
   python cyborg/restore_backup.py --auto
   ```
3. Интерактивное восстановление конкретного бэкапа:
   ```bash
   python cyborg/restore_backup.py
   ```
   Следовать подсказкам (выберите номер, подтвердите перезапись).
4. Проверить `state.json` после восстановления:
   ```bash
   cat idea_engine/data/state.json | jq '.ideas | length'
   ```
5. Если все бэкапы повреждены — последний resort: удалить `state.json`, kibорг создаст новый
   пустой инбокс (все идеи потеряны, но система продолжит работать).
...
### ✅ P3 — Auto-recovery в restore_backup (РЕШЕНО 2026-07-21)
- **Проблема:** при повреждении state.json нужен был ручной запуск `restore_backup.py`.
- **Решение:** модуль `cyborg/recover_state.py` с `auto_recover_state_if_needed()`.
  Вызывается из `harvest_runner.main()` в начале (после `ensure_data_dirs`, до `backup_state`).
  Если state.json повреждён/отсутствует и есть валидный бэкап — восстанавливает, шлёт
  CRITICAL-алерт, прогон продолжается. Повреждённый файл сохраняется как
  `state.json.corrupted-<TS>` для разбора. Restore CLI расширен флагом `--auto` для
  неинтерактивного запуска из cron/скриптов.
- **Frozen constraint соблюдён:** проверка идёт ДО передачи управления органам; `store.py`
  не тронут.
